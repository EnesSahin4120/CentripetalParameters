...
namespace NWH.Common.Cameras
{
    ...
    public class CameraChanger : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            _vehicle.onWake.AddListener(EnableCurrentDisableOthers);
            _vehicle.onSleep.AddListener(DisableAllCameras);
            ...
            _vehicle.onVehicleMultiplayerTypeChanged.AddListener(OnMultiplayerInstanceTypeChanged);
        }


        private void Update()
        {
            if (_vehicle.IsAwake && !_vehicle.GetMultiplayerIsRemote() && InputProvider.Instances.Count > 0)
            {
        ...
        {
            if (!_vehicle.IsAwake || _vehicle.GetMultiplayerIsRemote())
            {
                DisableAllCameras();
            }
            else
        ...
        {
            if (_vehicle.GetMultiplayerIsRemote())
            {
                return;
            }
