...
namespace NWH.Common.CoM
{
    ...
    public class VariableCenterOfMass : MonoBehaviour
    {
        ...
        public Vector3 CalculateInertiaTensorFromDimensions(Vector3 dimensions, VehicleType vehicleType)
        {
            Vector3 coefficient = new Vector3(0.16667f, 0.16667f, 0.16667f); // Coefficient for a cube

            if (vehicleType == VehicleType.Car)
            {
                coefficient = Vector3.Scale(coefficient, new Vector3(0.3f, 0.45f, 0.3f));
            }
            else if (vehicleType == VehicleType.Truck)
            {
                coefficient = Vector3.Scale(coefficient, new Vector3(0.5f, 0.7f, 0.5f));
            }
            else if (vehicleType == VehicleType.Aircraft)
            {
                coefficient = Vector3.Scale(coefficient, new Vector3(0.5f, 0.8f, 1f));
            }

            return new Vector3
                (
                    (dimensions.y * dimensions.y + dimensions.z * dimensions.z) * coefficient.x * combinedMass,
                    (dimensions.z * dimensions.z + dimensions.x * dimensions.x) * coefficient.y * combinedMass,
                    (dimensions.x * dimensions.x + dimensions.y * dimensions.y) * coefficient.z * combinedMass
                );
        }
        ...
        public override bool OnInspectorNUI()
        {
            ...
            if (!drawer.Field("useDefaultInertia").boolValue)
            {
                ...
                drawer.BeginSubsection("Calculate Inertia From Dimensions");
                {
                    ...
                    {
                        vcom.inertiaTensor = vcom.CalculateInertiaTensorFromDimensions(vcom.dimensions, vcom.vehicleType);
